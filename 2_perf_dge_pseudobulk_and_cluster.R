...
perf_combinations <- combn(perf_values, 2)
perf_combinations_df <- data.frame(group1 = perf_combinations[1, ], group2 = perf_combinations[2, ])

# To match Velsera Analyses
perf_combinations_df[3,]$group1 <- "OOS"
perf_combinations_df[3,]$group2 <- "Golden"
...
for(i in seq(nrow(perf_combinations_df))){
  ...
  de.results <- pseudoBulkDGE(
    ...
    # design = as.formula(paste0("~", group_condition)),
  ...
  current_sce <- sce_aggregated[,sce_aggregated[[group_condition]] %in% c(group1, group2)]
  dim(current_sce)
  colData(current_sce)[, c("FiralisID", "Batch", "Donor")]
...
params_for_post_process <- list(
  ...
  results_file_name = paste0(dge_results_directory, "All_DGE_results_total_pseudobulk.tsv")
)
...
params_for_post_process <- list(
  ...
  results_file_name = paste0(dge_results_directory, "All_DGE_results_per_cluster.tsv")
)
